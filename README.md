...
- [`scripts/extract_photo_summary.sh`](scripts/extract_photo_summary.sh): Extract key EXIF details, build a concise pipe-separated photo summary, and write it to `EXIF:ImageDescription` and `EXIF:UserComment`. AI-generated descriptions go in IPTC fields separately so the technical summary is never overwritten.
- [`scripts/annotate_photos_with_ollama.sh`](scripts/annotate_photos_with_ollama.sh): Run one Ollama workflow at a time: description mode writes to `IPTC:Caption-Abstract`, keywords mode populates empty `IPTC:Keywords`, headline mode populates empty `IPTC:Headline` (includes Adobe Stock-optimized title prompts). AI metadata stays in IPTC fields only — `EXIF:ImageDescription` is reserved for the technical photo summary. All workflows support prompt selection (`--list-prompts`, `--prompt-id`, `--prompt-file`).
