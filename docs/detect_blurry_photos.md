...
- split photos into scenes by EXIF timestamp gaps
...
# Split into scenes with 8-second gap and select sharpest from each
scripts/detect_blurry_photos.sh --mode select --time-gap 8 --clean
...
- Scene split uses `EXIF:DateTimeOriginal` first, then `EXIF:DateTime`, then file mtime fallback.
